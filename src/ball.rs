...
impl Ball {
	...
	pub fn check_will_through_paddle(&self, paddle: &mut Paddle) {
		if self.position.x <= paddle.position.x + 2 {
			for i in 0..paddle.height {
				if self.position.y + 1 == paddle.position.y + i {
					paddle.render();
				}
			}
		}
